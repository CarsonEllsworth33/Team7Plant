...
interface MyPlantDao {
    @Query("SELECT * FROM myplants ORDER BY name")
...
/*
    ...
    */
    ...
    @Query("SELECT * FROM myplants WHERE name LIKE :name")
    fun getPlant(name: String): LiveData<List<MyPlant>>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertAll(plants: List<MyPlant>)

    @Query("SELECT * FROM myplants WHERE id = :plantid")
    fun getPlantID(plantid: Int): LiveData<MyPlant>
